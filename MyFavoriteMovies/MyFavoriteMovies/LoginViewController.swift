...
class LoginViewController: UIViewController {
    ...
    private func getRequestToken() {
        ...
        let task = appDelegate.sharedSession.dataTaskWithRequest(request) { (data, response, error) in
            ...
            // was there a successful 2xx response
            guard let statusCode = (response as? NSHTTPURLResponse)?.statusCode where statusCode >= 200 && statusCode <= 299 else {
                displayError("Your request returned a status code other than 2xx!")
                return
            }
            
            // was the data returned
            ...
            // is the request token key in the parsedResult
    ...
    private func loginWithToken(requestToken: String) {
        ...
        let task = appDelegate.sharedSession.dataTaskWithRequest(request) { (data, response, error) in
            ...
            // Did TheMovieDB return an error?
            if let _ = parsedResult[Constants.TMDBResponseKeys.StatusCode] as? Int {
                displayError("The MovieDB returned an error. See the '\(Constants.TMDBResponseKeys.StatusCode)' and '\(Constants.TMDBResponseKeys.StatusMessage)' in \(parsedResult)")
                return 
            }
            
            // is the success key in the parsed result
    ...
    private func getSessionID(requestToken: String) {
        ...
        let task = appDelegate.sharedSession.dataTaskWithRequest(request) { (data, response, error) in
            ...
            // was there any data returned
            guard let data = data else {
                displayError("No data was returned for this request!")
                return
            }
            ...
            print("got the sessionId: \(sessionID)")
        }
    ...
    private func getUserID(sessionID: String) {
        ...
        let task = appDelegate.sharedSession.dataTaskWithRequest(request) { (data, response, error) in
            ...
            // was there any data returned
            guard let data = data else {
                displayError("No data wass returned by the request!")
                return
            }
